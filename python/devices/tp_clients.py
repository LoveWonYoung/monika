...
from lib.lintp_engine_ctypes import LinTpConfig, LinTpEngineWorker
...
class LinTpWorker:
    ...
    def start(self) -> None:
        ...
        if self._bridge_thread is not None and self._bridge_thread.is_alive():
            return
        self._stop_evt.clear()
        self._bridge_thread = threading.Thread(target=self._bridge_loop, name="HwLinTpBridge", daemon=True)
        self._bridge_thread.start()
