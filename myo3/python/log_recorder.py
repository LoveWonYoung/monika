...
def init_and_rotate(log_name: str, base_dir: str = ".", interval_minutes: int = 10) -> Path:
    ...
    with _LOG_LOCK:
        ...
        _ROTATE_THREAD = threading.Thread(
            target=_rotate_loop,
            args=(log_name, base_dir, float(interval_minutes * 60)),
            name="LogRotate",
            daemon=True,
        )
...
def stop_rotate(timeout_s: float = 1.0) -> None:
    global _ROTATE_THREAD

    _ROTATE_STOP_EVENT.set()
    with _LOG_LOCK:
        if _ROTATE_THREAD is not None:
            _ROTATE_THREAD.join(timeout=timeout_s)
            _ROTATE_THREAD = None
